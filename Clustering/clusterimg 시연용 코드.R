...
# pch: 모양, col: 색
# 모든 변수들 간 plot 확인
plot(data, pch = kmeans_scaled$cluster, col = kmeans_scaled$cluster)
...
kmeans5 <- kmeans(data1, 4, 10)
kmeans5$cluster
result_table <- table(kmeans5$cluster, data$Species)
result_table
sum(50, 23, 27, 27) / sum(result_table) * 100

# 3. H-Clust 함수 적용시켜보기
# 위에서 스케일된 데이터를 hclust 함수에 적용시켜보자
data_hclust <- data1
data_hclust
distance <- round(dist(data_hclust), 2)
