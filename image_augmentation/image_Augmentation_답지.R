...
shift <- function(img, cord){
  ...
  if(cord[1] > 0){
    for(i in 1:3){
      img[,,i] <- rbind(matrix(0, nrow = cord[1], ncol = iw), img[1:(ih-cord[1]),,i])
    }
  }
  if(cord[1] < 0){
    for(i in 1:3){
      img[,,i] <- rbind(img[(1-cord[1]):ih,,i], matrix(0, nrow = -1 * cord[1], ncol = iw))
    }
  }
  if(cord[2] > 0){
    for(i in 1:3){
      img[,,i] <- cbind(matrix(0, nrow = ih, ncol = cord[2]), img[,1:(iw-cord[2]),i])
    }
  }
  if(cord[2] < 0){
    for(i in 1:3){
      img[,,i] <- cbind(img[,(1-cord[2]):iw,i], matrix(0, nrow = ih, ncol = -1 * cord[2]))
    }
  }
  return(img)
}
...
rotate <- function(img, angle){
  ...
  if(angle %% 360 == 90){
    timg <- array(0, c(iw, ih, 3))
    for(k in 1:3){
      for(i in 1:ih){
        for(j in 1:iw){
          timg[j,1+ih-i,k] <- (img[i,j,k])
        }
      }
    }
    return(timg)
  ...
  if(angle %% 360 == 180){
    timg <- rotate(rotate(img, 90), 90)
    return(timg)
  }
  if(angle %% 360 == 270){
    timg <- rotate(rotate(rotate(img, 90), 90), 90)
    return(timg)
  }
}
...
{
  makeBoundary <- function(image, color = 1)
  {
    ori_img <- image
    new_img <- array(0, c(dim(image)[1]+color*2, dim(image)[2]+color*2,3))
    for(z in 1:3){
      image <- ori_img[,,z]
      colname <- names(image)
      height <- dim(image)[1]
      width <- dim(image)[2]
      image <- rbind(rep(color, width), image, rep(color, width))
      image <- cbind(rep(color, height + 2), image, rep(color, height + 2))
      new_img[,,z] <- image
    }
    return(new_img)
  }
  
  ## calculatePixel : 주어진 픽셀값을 filter 연산을 해주는 함수
  calculatePixel <- function(image, y, x, filter)
  {
    value <- 0
    clockwise <- list(c(-1,-1), c(0,-1), c(1,-1), c(1,0), c(1,1), c(0,1), c(-1,1), c(-1,0), c(0,0)) # 방향을 미리 설정해주고 반복문을 통해 불러온다.
    for(i in 1:9)
    {
      value <- value + image[y+clockwise[[i]][1], x+clockwise[[i]][2]] * filter[clockwise[[i]][1] + 2, clockwise[[i]][2] + 2]
    }
    return(value)
  }
  image <- makeBoundary(image)
  height <- dim(image)[1]
  width <- dim(image)[2]
  outputImage <- array(0, c(height-2, width-2, 3))
  for(z in 1:3){
    for(y in 2:(height-1)) # 테두리를 제외한 각 셀마다 calculatePixel 함수를 이용하여 새로운 이미지 매트릭스(outputImage)를 만든다.
    {
      for(x in 2:(width-1))
      {
        outputImage[y-1, x-1,z] <- calculatePixel(image[,,z], y, x, filter)
        if(outputImage[y-1, x-1,z] < 0){
          outputImage[y-1, x-1,z] <- 0
        }
        if(outputImage[y-1, x-1,z] > 1){
          outputImage[y-1, x-1,z] <- 1
        }
      }
    }
  }
